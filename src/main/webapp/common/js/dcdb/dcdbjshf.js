...
$(function () {
    ...
    $('.download-files').on('click',function(){
        ...
        if(ids != ''){
            $.download('/dcdb/filesDowload.do',{'ids':ids,'nodeId':'3','id':$('#ajid').val()});
        }
        else{
    ...
    $('.delete-files').on('click',function(){
        ...
        if(ids != ''){
            layer.confirm("确定删除？", {
                ...
                function(){
                    $.commonAjax('/dcdb/filesDelete.do',{'ids':ids,'nodeId':'3','id':$('#ajid').val()},param.deleteFiles);
                }, //第二个按钮的方法
                function(){
                    layer.closeAll();
                });
