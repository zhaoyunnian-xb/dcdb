...
function selectPerson(sftg) {
   ...
   //当发送内勤节点时，设置成多选，flag设置成nq，显示内勤人员
    ...
    if ((ywtype == "一般性工作批示"  && nodeid == "0105" && fzlcbs =="DCS") || (ywtype == "专项督办省院"  && nodeid == "0105" && fzlcbs =="DCS")) {
      flow(sftg,"",isBatch);
      return;
    }
           ...
           data.push(parmBlqk);
            ...
            //督查审核人-意见数据
             ...
             $("[name='dbsfhr']").each(function () {
               ...
               // parm5[name]=$('#'+ id).val();
               parmDcshr[name] = validate(nodeid, '01011') ? $('#' + id).val() : '';
             })
             data.push(parmDcshr);

     //针对： 一般性批示、专项督办省院、专项督办市院
    }else{
      if(nodeid <= "0104"){
        var parmBlqk=getParm(id,ywtype,"blqk");
            parmBlqk.spbz=blqk;
          ...
          if (nodeid == "0104") {
            ...
            $("[name='dscsp']").each(function () {
              ...
              //parm1[name]=$('#'+ id).val();
              if (ywtype == '专项督办市院') {
                parmDcfhr[name] = validate(nodeid, '0107') ? $('#' + id).val() : '';
              } else {
            ...
            //院领导-意见数据
            var parmYld = getParm(id, ywtype, "yldsp");
            $("[name='yldsp']").each(function () {
              var id = $(this).attr('id');
              var name = $(this).attr('id').split("_")[1];
              // parm3[name]=$('#'+ id).val();
              parmYld[name] = validate(nodeid, '0106') ? $('#' + id).val() : '';
            })
            data.push(parmDcfhr);
            data.push(parmBmfzr);
            data.push(parmYld);
        ...
        $("[name='dbsfhr']").each(function () {
          ...
          // parm5[name]=$('#'+ id).val();
          if(ywtype == '专项督办市院') {
            parmDcshr[name] = validate(nodeid, '01071') ? $('#' + id).val() : '';
          }else{
...
function initBtn() {
    ...
    if(ywtype == "专项督办省院" || ywtype == "一般性工作批示" || ywtype == "专项督办市院" ){
        $("#submitFlow").show();
        $("#rebackFlow").hide();
        $("#submitFlowFzr").hide();
        $("#yulan").hide();
        ...
        }else if((nodeid == '0106' && ywtype!='专项督办市院') || (nodeid == '0107' && ywtype=='专项督办市院')){
            //督查室复核
            $("#rebackFlow").show();
            $("#submitFlow").html("复核通过");
        }else if(nodeid == '0106' && ywtype=='专项督办市院'){
            //院领导审核（专项督办市院）
            $("#rebackFlow").show();
            $("#submitFlow").html("审核通过");
        } else if(nodeid == "8888"){
        ...
        }else if((nodeid == '01061' && ywtype!='专项督办市院') || (nodeid == '01071' && ywtype=='专项督办市院')){
            //督查审核
            $("#submitFlow").html("审核通过");
            $("#rebackFlow").show();
        }
      ...
      $("#dscsp_spr").val(  $("#dscsp_spr").val() == $("#curname").val() ?$("#dscsp_spr").val() : "");
        $("#dscsp_spr").val(  $("#dscsp_spr").val() == "" ?$("#curname").val() : $("#dscsp_spr").val());
         ...
         laydate.render({
             elem: '#yldsp_sprq',
             format: 'yyyy年MM月dd日',
             value: $("#yldsp_sprq").val() == "" ? new Date() : $("#yldsp_sprq").val(),
             isInitValue: true,
             done: function(value, date, endDate){
                 $("#yldsp_sprq").val(value);
             }
         });
         $("#yldsp_spr").val(  $("#yldsp_spr").val() == $("#curname").val() ?$("#yldsp_spr").val() : "");
         $("#yldsp_spr").val(  $("#yldsp_spr").val() == "" ?$("#curname").val() : $("#yldsp_spr").val());
     }else if((nodeid == '01061' && ywtype!='专项督办市院') || (nodeid == '01071' && ywtype=='专项督办市院') || (nodeid == '01011' && (ywtype=='编发要情' || ywtype=='归档'))){
        //督查审核
        laydate.render({
            elem: '#dbsfhr_sprq',
            format: 'yyyy年MM月dd日',
...
function spyjHideAndDis() {
    ...
    var ywtype = $("#bllxyc").val() == "" ? $('#bllx option:selected').val() : $("#bllxyc").val();
    if(ywtype == "归档" || ywtype=="编发要情"){
        $(".layui-colla-item :eq(0)").siblings().addClass("layui-hide");
        $(".layui-colla-item :eq(3)").removeClass("layui-hide")
    }else if(ywtype == "一般性工作批示" ||ywtype == "专项督办省院" ){
        $(".layui-colla-item :eq(1)").addClass("layui-hide");
    }

    if(nodeid == '0101'){
        $(".layui-colla-item").find("input,textarea").attr('disabled', 'true');
    }else if(nodeid == '01011' && (ywtype == "一般性工作批示" ||ywtype == "专项督办省院" ||ywtype == "专项督办市院" )){
        $(".layui-colla-item").find("input,textarea").attr('disabled', 'true');
    }else if(nodeid == '0102'){
        //办公室负责人审批-拟办送审
        ...
        //督查室复核
        $(".layui-colla-item :eq(4)").siblings().find("input,textarea").attr('disabled', 'true');
    }else if(nodeid == '0106' && ywtype=='专项督办市院'){
        //院领导审批（专项督办市院）
