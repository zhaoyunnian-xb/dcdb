...
layui.define(["jquery","laytpl"], function (exports) {
    ...
    Class.prototype={
        ...
        ajaxGetData: function() {
            ...
            var data = $.extend(true,{}, options.where );
        ...
        eleTreeEvent: function() {
            ...
            options.elem.on('dblclick',expandOnClickNode, function (e) {
                debugger;
                $(this).off('click')
            } )
        },
        ...
        expandAll: function(data,arr,floor,isMethodsExpandAll) {
            var options=this.config;
            var _self=this;
            data.myForeach(function(val,index) {
                arr.push(index);
        ...
        defaultChecked: function() {
            ...
            // 判断是否父子无关
            if(options.checkStrictly){
                options.defaultCheckedKeys.myForeach(function(val,index) {
        ...
        search: function(value) {
            ...
            var traverse=function(data) {
                data.myForeach(function(val,index) {
                    ...
                    // 删除子层属性
                    if(val[options.request.children] && val[options.request.children].length>0){
                        val[options.request.children].myForeach(function(v,i) {
                            delete v.visible;
                        })
                    }
                })
